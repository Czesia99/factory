...
namespace sigel
{
    ...
    void VertexManager::createVertexBuffer(std::vector<Vertex> vertices, vk::raii::CommandPool &pool)
    {
        ...
        vk::raii::Buffer stagingBuffer(_lDevice->getDevice(), stagingInfo);
        ...
        vk::raii::DeviceMemory stagingBufferMemory(_lDevice->getDevice(), memoryAllocateInfoStaging);
    ...
    void VertexManager::copyBuffer(vk::raii::CommandPool &pool, vk::raii::Buffer &srcBuffer, vk::raii::Buffer &dstBuffer, vk::DeviceSize size)
    {
        ...
        vk::raii::CommandBuffer commandCopyBuffer = std::move(_lDevice->getDevice().allocateCommandBuffers(allocInfo).front());
        commandCopyBuffer.begin(vk::CommandBufferBeginInfo { .flags = vk::CommandBufferUsageFlagBits::eOneTimeSubmit });
        commandCopyBuffer.copyBuffer(srcBuffer, dstBuffer, vk::BufferCopy(0, 0, size));
        commandCopyBuffer.end();
        _lDevice->graphicsQueue.submit(vk::SubmitInfo{ .commandBufferCount = 1, .pCommandBuffers = &*commandCopyBuffer }, nullptr);
        _lDevice->graphicsQueue.waitIdle();
    }
}